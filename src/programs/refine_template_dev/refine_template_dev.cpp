...
#pragma omp for schedule(dynamic, 100)
        for ( long match_id = arguments.start_position - 1; match_id < arguments.end_position; match_id++ ) {
            ...
            if ( current_image_filename.compare(input_matches.all_parameters[match_id].original_image_filename.ToStdString( )) != 0 ) {
                ...
                //average /= num;
                //for ( int i = 0; i < whitening_filter.number_of_points; i++ ) {
                //    whitening_filter.data_y[i] = whitening_filter.data_x[i] / (whitening_filter.data_x[i] + 0.025);
                //}
                //whitening_filter.MultiplyByConstant(1.0f / whitening_filter.ReturnMaximumValue( ));
                //whitening_filter.WriteToFile(wxString::Format("whitening_filter_km_%s.txt", wxFileName(input_matches.all_parameters[match_id].original_image_filename).GetName( )));
