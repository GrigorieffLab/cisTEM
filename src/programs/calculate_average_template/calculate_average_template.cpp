...
void CalculateAverageTemplateApp::DoInteractiveUserInput( ) {
    ...
    apply_CTF             = true; // todo add this option
    apply_shifts          = false;
    ...
    my_current_job.ManualSetArguments("ttbtfffftbbbbffiff",
...
bool CalculateAverageTemplateApp::DoCalculation( ) {
    ...
    global_euler_search.CalculateGridSearchPositions(false); // todo: what whitening filter? -> can be applied in compare_template
...
#pragma omp parallel for num_threads(max_threads) default(shared) private(current_angular_position, current_psi, input_parameters, my_parameters, my_ctf, variance) firstprivate(projection_image, final_image, final_image_squared)
    for ( current_angular_position = first_search_position; current_angular_position <= last_search_position; current_angular_position++ ) {
